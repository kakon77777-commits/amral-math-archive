---
title: "Navier–Stokes C6-O：Peak-Scale Defect Inheritance、Type-II Ancient Carriers 與 Mass–Peak Two-Scale Closure"
...
# Navier–Stokes C6-O
# Peak-Scale Defect Inheritance、Type-II Ancient Carriers 與 Mass–Peak Two-Scale Closure

## 0. 本輪定位

C6-N 將 singular carrier重新分成：
...
其中 Type-II relative-dominant carrier：
...
在 record peak scale做 exact N–S rescaling：
...
$$
\boxed{
\textbf{bounded ancient peak profile是 global }L^3
...
> **如果 global critical mass都跑到 ancient peak frame的 spatial infinity，
> TS / GP / HF defect label能不能仍留在 bounded peak core？**
...
1. 定義 mass/peak scale ratio：
   $$
   \mathfrak R_n^{MP}
   =
   A_n\ell_n;
   $$
2. Type-II relative dominance給：
   $$
...
3. global normalized：
   ...
   L^3
   $$
   mass probability在 peak frame對每個 fixed ball都趨零；
4. 因此：
   $$
   \boxed{
   ...
   $$
5. 定義 independent peak defect tightness：
   $$
   \Theta_D^{peak};
   $$
6. defect label可：
   ...
   - partial；
   - peak-escape；
7. 若 defect label peak-tight，
   它必與 global relative：
   $$
   L^3
   $$
   probability asymptotically decouple；
8. 因此 ancient defect inheritance必使用：
   $$
   \boxed{
   \textbf{local absolute defect load};
   }
   $$
9. 建立 general Local Absolute Defect Inheritance Lemma；
10. dimensionless local margins可在 strong local convergence下傳給 ancient profile；
11. global-normalized carrier probabilities不一定傳；
12. 定義 derivative-to-peak ratio：
    $$
    \widehat A_{k,n}^{peak}
    =
    A_{k,n}/A_{0,n}^{k+1};
...
17. 對：
    ...
    k=1,
    $$
    peak ancient profile在 spatial variables中變 flat/constant；
18. HF global-threshold labels只有在 derivative amplitude被 peak frame捕獲時才能合法繼承；
19. TS需要額外：
    $$
    \boxed{
    ...
    $$
20. GP local geometry可以在：
    $$
    C^2_{\rm loc}
    $$
    convergence + nondegenerate Q mass/margins下繼承；
21. far-pressure provenance必重新 partition；
22. record peak frame：
    $$
    \|v_n(\tau)\|_\infty\le1;
    $$
23. remote pressure-Hessian source：
    $$
    |y|>R
    $$
    對 fixed peak core的 contribution：
    $$
    \boxed{
    O(R^{-2});
    }
    $$
24. 因此 mass-scale tail雖承擔幾乎全部 global relative：
    $$
    L^3
    $$
    mass，卻無法維持：
...
25. 這是：
    ...
    $$
26. Type-II ancient peak pressure provenance因此變成 peak-local finite-radius problem；
27. recent 2026 Seregin Type-II work提供另一個 conditional external interface：
    certain Type-II scenarios可經 Euler scaling產生 ancient Euler limit並使用 Euler Liouville analysis；
28. C6-O不把該 scenario theorem套成 universal N–S Type-II kill；
29. current Type-II carrier最後分成：
    $$
    \boxed{
    ...
    $$
30. Ancient peak profile若繼承 defect label，
    才能合法接：
    - ancient Liouville；
    - Type-I decay；
    - ancient GP/HF/TS rigidity；
31. 若 label peak-escapes，
    ancient profile與 C6 defect cycle必視為兩個不同 carriers。
