...
- Previous round: `NS_X72_Round46_PureContinuous_InvisibleEscape_AmplitudeBeltramiTension_v0.1_2026-08-17.md`
- This round's objective: Round 46 has precisely scalarized asymptotic invisibility into
  $$
  \Theta_\omega
  =
  A_\omega
  ...
  $$
  This round investigates the dynamics of this cancellation: deriving the amplitude source, total visibility-defect source, tension source, anti-coherence dynamics, source-lock hierarchy, critical forcing budget, and the linearized normal operator near a constant-amplitude Beltrami field.
- Non-claims: This document does not claim that the amplitude–tension cancellation cannot persist long-term. What this document proves is: in addition to the state lock
  $$
  ...
  F_A+F_T\approx0
  $$
  is also required. The invisibility contact order of non-invariant branches is determined by the successive time jets of $\Theta_\omega$; the energy-level closure remains stuck at the scale-critical strain / vorticity-gradient / transport-commutator budget.
...
Thus, the amplitude carrier is driven by:
...
Therefore, there is no need to directly expand:
...
Applying a scalar Riesz projection directly to the stress PDE is much cleaner.
...
Equation (3.3) decomposes the invisibility-defect source into:
...
Therefore, the amplitude and tension carriers obey the same convection–diffusion backbone,

but possess different sources:
...
the linear effects of advection and diffusion on both sides are automatically compatible.

What truly determines whether the lock breaks in the next instant is:
...
Therefore, persistent pure invisibility additionally requires:
...
define the source anti-coherence:
...
Therefore, persistent invisibility requires simultaneously:
...
Define the normalized amplitude growth:
...
The cross inner product satisfies:
...
This is the exact amplitude–tension anti-coherence equation.
...
Therefore, linear advection–diffusion itself does not break the lock.
...
will push the state off the perfect cancellation manifold.
...
This is the scalar energy ledger for the entire visibility-cancellation dynamics.
...
The Round 45 visible energy equation is therefore equivalent to:
...
Thus, the source channels satisfy exact energy projection identities.

In particular, the transport term:
...
This proves once again that the transport commutator is a visibility transfer, rather than total stress creation.
...
The Round 46 second-order injection therefore becomes:
...
Thus, the boundary ejection curvature is completely decomposed into the stretching / gradient / transport source mismatch.
...
Therefore, viscosity indeed damps the cancellation defect,

but a critical source mismatch can continuously regenerate it.
...
So the Source-Lock Critical Exposure is controlled conditionally by:
...
Thus, the source-lock closure reconnects to the Round 37 critical strain and Round 18/42 vorticity-gradient budgets.
...
Therefore, the projected gradient-stress forcing still spends a second-vorticity-derivative budget in this straightforward route.
...
This explains the Round 45–46 pure-invisible Beltrami branch as a genuine invariant cancellation manifold, not accidental finite-order fine tuning.
...
Take a divergence-free perturbation:
...
- helical mode enumeration as a proof substrate;
...
is merely an ordinary smooth-time Taylor order used to classify a local vanishing event,

and does not constitute an essential discrete substrate; all proof equations remain on the continuous time/PDE carrier.
...
If the first nonzero visibility carrier time jet is of order:
