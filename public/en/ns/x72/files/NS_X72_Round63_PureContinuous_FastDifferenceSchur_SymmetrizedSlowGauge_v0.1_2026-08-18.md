...
- Date: 2026-08-18
- Version: v0.1
- Status: Proof-Route Experiment / Continuous-Only Exact Fast-Elimination Branch
- canonical source: UTF-8 Markdown
- canonical math delimiters: inline `$...$`; display `$$...$$`
- Previous round: `NS_X72_Round62_PureContinuous_NeutralCancellation_RestoredQuarterPowerMatching_v0.1_2026-08-18.md`
- Objective of this round: Round 62 compressed the small-viscosity stable-bundle gap into
  $$
  \text{fast Schur graph}
  +
  \text{slow Riccati}.
  $$
  This round seeks an exact Schur coordinate that does not rely on an ill-conditioned local eigenbasis, directly eliminating the fast sector from the original third-order parity recurrence.
- Main results:
  1. Using the first-order differences
     $$
     d^E_j=E_j-E_{j-1},
     \qquad
     d^o_j=o_j-o_{j-1}
     $$
     as fast variables, the third-order recurrence for each parity can be exactly rewritten as a "slow forcing + contractive fast feedback" equation;
  2. The fast feedback coefficients
     $$
     p_j,\ q_j
     $$
     are completely independent of viscosity;
  3. Directly from the exact coefficient boxes certified in Round 59, for all
     $$
     j\ge10
     $$
    :
     ...
     $$
     on the large fibre;
  4. Therefore, the fast difference operator
     $$
     I-\mathcal K_{\rm fast}
     $$
     can be rigorously inverted by a Neumann series, and
     $$
     \boxed{
     \|(I-\mathcal K_-)^{-1}\|
     <
     ...
     $$
  5. The fast sector can thus be **exactly slaved** to the slow variables;
  6. The exact Schur elimination can be written as
     $$
     \boxed{
     d^E
     =
     ...
     $$
  7. The fast resolvent itself is viscosity-independent; the difference between positive viscosity and the endpoint only enters the slow forcing, not the fast inversion;
  8. Coefficient asymptotics:
     ...
     \right);
     $$
  9. Define the symmetrized coupling and staggered gauge:
     $$
     \boxed{
     \bar c_j
     =
  ...
  10. Then
      ...
      $$
  11. Therefore, the $1/j$ asymmetry of the two-parity coupling in Round 62 is mostly a removable staggered-gauge effect; after
      $$
      j\mapsto j+\frac34
      $$
      and a diagonal gauge, the first nontrivial mismatch of the slow coupling drops to
      $$
      O(j^{-2});
      $$
  12. At this point, the "fast Schur graph" is no longer an open object; the remaining singular gap lies only in the **symmetrized slow Jost/Riccati scattering / minimal selection**;
  13. A full numerical coefficient audit shows that the actual fast feedback is much smaller than the coarse rigorous bounds above; at $j=10$:
      - small fibre:
        $$
      ...
      - large fibre:
        ...
        $$
- Non-claims: This round does not prove the slow Jost/Riccati minimal-selection matching, nor does it yet obtain a complete positive-viscosity interval. It rigorously completes the **operator-level elimination of the fast sector** and identifies the natural symmetrized gauge of the coupling.
