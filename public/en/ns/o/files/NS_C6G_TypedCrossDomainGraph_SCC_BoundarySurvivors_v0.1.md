---
title: "Navier–Stokes C6-G: Typed Cross-Domain Graph Rebuild, Joint-Node SCC Audit and Minimal Boundary-Saturated Survivor Cycles"
...
# Navier–Stokes C6-G
# Typed Cross-Domain Graph Rebuild, Joint-Node SCC Audit and Minimal Boundary-Saturated Survivor Cycles

## 0. Positioning of this Round

The coarse residual graph of C5-M uses:
...
At that time, the ordinary may-graph easily generated:
...
This was merely a useful first-layer compression.

C6-A subsequently pointed out:
...
C6-B–F then sequentially reworked the cycle semantics:
...
was proven to be mostly same-event compatibility,
...
was proven to be merely the temporal marginal of the spacetime source state,
...
C6-F further established the first batch of genuine cross-domain typed bridges:
...
Therefore, C6-G now rebuilds the graph,
...
1. The large coarse SCC of C5-M:
   $$
   \{G,P,H,F\}
   $$
   dissolves after typed correction;
2. The static G/P loop collapses into the joint node:
   $$
   GP;
   $$
3. The H/F loop collapses into the coherent nonlinear re-entry node:
   $$
   HF;
   $$
4. The T trap is lifted to the shared-source joint node:
   $$
   TS;
   $$
5. The refined interior node set:
   $$
   \boxed{
   V_{\rm int}
   =
   ...
   $$
6. The certified/conditional cross-domain directions are primarily:
   $$
   \boxed{
   TS\to GP,
   \qquad
   ...
   $$
7. Currently there are no certified:
   $$
   GP\to TS,
   \quad
   GP\to HF,
   ...
   HF\to GP;
   $$
8. $GP\to GP$ recurrence still requires a geometry+pressure heredity theorem;
9. $HF\to HF$ recurrence still requires a nonlinear coherent re-entry theorem;
...
14. The current typed dynamic graph has:
    $$
    \boxed{
...
16. The minimal survivor frontier becomes:
    $$
    \boxed{
...
large temporal variation is needed to destroy a strict sign margin,
...
C5-M once compressed the residual physics into:

$$
A,T,G,P,H,F.
$$

That coarse may-graph appeared to contain a large:
...
After item-by-item inspection in C6-A–F,

C6-G can now formally state:

$$
\boxed{
\textbf{This large SCC has dissolved under typed dynamic semantics.}
}
$$
...
is mostly same-event compatibility,

so it is quotiented into:
...
only the nonlinear coherent re-entry subtype can possibly form a genuine recurrence,
...
is not a complete physical state,
...
the temporal marginal of the spacetime source state.

Fourth,

C6-F established for the first time:
...
But we have no certified reverse cross-domain edges.

So the current interior graph is:
...
only have unproven self-recurrence obligations.

Therefore:

$$
\boxed{
\textbf{Currently there is no nontrivial certified interior SCC.}
}
$$

Even all current candidate cycles have:
...
But this is not a regularity proof.

It merely tells us:

> **The truly unknown global recurrence is no longer hidden in the coarse interior graph.**

If a hypothetical survivor cannot form a uniformly hereditary:

$$
GP
$$

or uniformly coherent:

$$
HF,
$$

then it must repeatedly approach a critical boundary face.

C6-G compresses all these boundaries further into a finite alphabet:
...
So the current true minimal survivor reduction is:
...
has disappeared from the minimal interior sink list.

This indicates that the next frontier for C6 is very clear:

$$
\boxed{
\textbf{It is no longer about finding an interior SCC,
but investigating whether critical boundary faces can themselves form a recurrent SCC.}
}
$$
