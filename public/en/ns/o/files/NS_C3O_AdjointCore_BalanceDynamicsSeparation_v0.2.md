...
The core revisions in this version relative to v0.1 are not merely stylistic polish, but rather theorem-safety enhancements:

- Eliminated the notational collision between the velocity-gradient $G=\nabla u$ and the integrated amplification $A_I$;
- Formulated the Betchov current and pressure current relied upon in C3-N as pointwise divergence lemmas, allowing the main balance of C3-O to be directly verified within this text;
...
- Refined the cancellation-precision debt into an exact residual trichotomy, avoiding statements like "not tending to zero at the same rate" which are insufficient to imply divergence;
...
- Updated the reference to Miller's conditional blow-up theorem to reflect current paper numbering and provide a precise description of the actual perturbative ratio, explicitly distinguishing it from the $\dot H^{-1}$ diagnostic proposed in this paper.
...
- $\rho\to-1^+$:
  Not excluded, but requires paying an increasingly precise gross cancellation;
- $\rho\to0$:
...
Unless otherwise stated, this paper works in $\mathbb R^3$, letting
...
To ensure that all integration by parts, homogeneous Sobolev norms, and terminal-value adjoint constructions are free of technical ambiguities, each identity can first be understood within the class of smooth rapidly decaying solutions; subsequently, extending them to standard strong/mild solution classes requires term-by-term continuation via density/approximation.

For the terminal cutoff, this paper takes
...
From $\nabla\cdot u=0$ and the commutation of mixed derivatives, we obtain
...
Thus, we have the pointwise identity
...
This is the precise origin of the localized Betchov conversion in this paper; its whole-space averaged version is consistent with the classical Betchov relation [3,5].
...
Thus, the starting identity of C3-O does not need to be treated merely as a black-box input from C3-N; its pointwise current mechanism can be directly verified within this text.
...
In the smooth setting described above, the terminal-value problem becomes a forward uniformly parabolic problem upon time reversal, thus possessing a unique smooth solution. The maximum principle gives
...
This paper refers to it as the:
...
Directly substitute into the localized strain balance from C3-N.
...
the first entire group of scalar cutoff terms exactly vanishes. $\square$
...
In a window where $A_I>0$, define:
...
Let $L^2_{st}$ denote the closed strain subspace formed by the symmetric gradients of divergence-free velocity fields, and let $P_{st}$ denote the orthogonal projection from $L^2$ to $L^2_{st}$.
...
This fact can be directly verified in the current language of this paper. Since $P_{st}$ is an orthogonal projection and $S\in L^2_{st}$,
...
And Lemma 1B.1, after integration over the whole space, gives the global Betchov cancellation
...
Under the smooth/decay assumptions of this manuscript, this orthogonality is therefore not an additional assumption.
...
Therefore, the more precise conclusion is
...
In particular, the whole-space constant choice $\chi\equiv1$ is a nonlocalized global solution to the adjoint equation (not in the aforementioned compact terminal cutoff class), and gives

$$
B_{\chi\equiv1}=0
$$

identically; but the full Navier–Stokes strain equation still contains $\mathcal P_{NS}$. Thus, in any whole-space window with $A_I>0$, the ratio is always
...
Therefore, what this paper obtains is an information-theoretic / structural no-go:
...
This is not claiming to have disproved approximation theorems under all possible additional assumptions using a specific explicit singular/full-NS counterexample; it merely rules out the route of **determining operator closeness solely from $\rho$**. $\square$
...
- Shares the same enstrophy-growth identity;
...
The perturbative hypothesis of that theorem is not the $\mathfrak P_I$ of this paper; it requires a pointwise-in-time $L^2$ ratio to remain controlled, with the denominator containing both $-\Delta S$ and nonlinear terms. In the notation of that paper, its core ratio takes the form
...
Combined with the initial-data sign condition specified in that paper, it deduces finite-time blow-up.

Therefore, what C3-O should preserve is not a crude statement like "small perturbation automatically means danger", but rather:
...
A certain specific relative closeness to the SSA dynamics can appear in a conditional blow-up theorem; on the other hand, other interaction/depletion structures might support regularity. Thus, one cannot treat a "small omitted term" or "large omitted term" itself as a monotonic regularity parameter.
...
Thus, $\mathfrak P_I^{\rm glob}$ can be understood as the time-integrated squared ratio of the omitted operator relative to the viscous operator.

However, it is **not a spatially localized diagnostic**. Therefore, v0.1 calling $(\rho_I,\mathfrak P_I)$ directly the "true local state" was too strong.
...
# 22. Note: operator diagnostics are still just candidates
...
they can be depletion mechanisms}.
}
$$
...
This paper refers to this as the:
...
If a certain **localized** operator defect $\mathfrak P_n^{\rm loc}\to0$, can one rigorously prove that the rescaled ancestry dynamics approach the SSA model?
...
Must preserve the guard: helical decomposition is inherently a spectral/nonlocal representation, and cannot be directly called a physical-space "local helical split".
...
Analyze the effective radius, Gaussian/Aronson-type tails, drift distortion, and pressure sensitivity of the terminal ancestry cutoff towards earlier times; backward persistence of compact support must not be assumed.
...
All C3-O ratio theorems are smooth/pre-singular window identities or their algebraic consequences; this paper does not elevate them into weak-solution singularity theorems.

## T-RATIO-DOMAIN

$\rho_I=B_I/A_I$ is only used in the ratio branch where $A_I>0$. Positive-growth windows with $A_I\le0$ are separately classified into the boundary-current-driven branch.
...
Miller's conditional blow-up theorem uses a specific $L^2$ relative perturbation hypothesis; it must not be conflated with the $\dot H^{-1}$ diagnostic in this paper.

## T-LOCALITY

$\mathfrak P_I^{\rm glob}$ is a whole-space/window diagnostic; $\mathfrak P_{I,\chi}^{\rm mult}$ is merely a cutoff-weighted candidate, as $\dot H^{-1}$ remains nonlocal. The true C3-P local stability route still requires establishing $\mathfrak P^{\rm loc}$ and cutoff/projection commutator control.
