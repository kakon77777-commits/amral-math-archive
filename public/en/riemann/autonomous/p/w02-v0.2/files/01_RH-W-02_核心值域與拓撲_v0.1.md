# RH-W-02: Core, Range, and Topology of Weil Test Functions
## Riemann Hypothesis GAP Engineering Note v0.1

**Research Project:** RH GAP Atlas / AI Mathematical Engineering Relay  
**Parent Node:** `RH-W-02`  
**Prerequisite Node:** `RH-W-01`  
**Status:** `CORE_CLOSED / GLOBAL_BRIDGE_OPEN`  
**Date:** 2026-07-23  
**Nature:** Function space and closure engineering; not a proof of RH, establishes no new Weil positivity results
...
Meanwhile, this round accomplishes the following under the LF topology of $C_c^\infty$:
...
However, this still does not equate to RH. The truly unfinished bridge is:
...
# 1. Three Layers of Spaces, Not to be Confused
...
Bombieri and Clay's reference class is the decay-type function space $\mathcal W$. The Weil criterion is formulated for $g\in\mathcal W$ satisfying two vanishing moments. This layer contains functions that are not compactly supported, so arithmetic sums, sums over zeros, and tail controls are no longer automatically finite.

## 1.3 The Strip Analytic Completion Layer

Lagarias uses the strip analytic test function space $\mathcal A_\delta$ on the Mellin side, equipped with the uniform norm on the closed strip as its topology; unconditionally, for $\delta>\tfrac12$, the Weil distribution acts as a continuous linear functional, and the Weil criterion can be formulated on an appropriate $\mathcal A_\delta$.

This project tentatively considers it as the candidate global target:
...
Moreover, $P$ is injective on $C_c^\infty(\mathbb R)$, hence:
...
the integral is zero when $u$ exceeds the right endpoint of the support of $G$; the integral is also zero when $u$ is to the left of the support. Therefore:
...
The two vanishing moments precisely ensure that $y$ and $h$ return to zero to the right of the support, so $h$ remains a compactly supported smooth function.

**Engineering Significance:** Every valid core input $g$ has a unique seed $h$, rather than only a subset of $g$ being generable by the operator.
...
So $P$ not only preserves compact support, but the inverse operator also does not need to expand the support.
...
or, after unifying the indices, written as:
...
And $P$ is clearly a continuous differential operator. Therefore:
...
The previous round's program used translations, scalings, modulations, and finite linear combinations of a fixed smooth bump. Now we distinguish:
...
This family is already exactly equal to the complete core and does not require a density argument.
...
The reason is: for any $H\in C_c^\infty$, the convolution $H*\eta_\varepsilon$ converges uniformly to $H$ in all derivatives; and each convolution integral can be approximated by finite Riemann sums in any finite-order $C^m$ seminorm. Handling all $m$ simultaneously with a diagonal sequence yields LF convergence.
...
The modulation parameter $e^{i\tau u}$ is not necessary for density, but it can enhance the frequency localization capability of the search dictionary.
...
then for any $g\in\mathcal C_{00}$, taking $g_n\in\mathcal D_\eta$ such that $g_n\to g$:
...
This corrects the vague issue of "transferring positivity using lower semi-continuity" in the old draft; this layer uses explicit quadratic form continuity.
...
But one cannot therefore directly extend the Weil quadratic form to bare $L^2$.

Reason 1: $L^2$ elements are only almost-everywhere equivalence classes; single-point evaluations are undefined.

Reason 2: Single-point evaluation is not continuous on $L^2(\mathbb R)$. Take a fixed smooth $\psi$ with $\psi(0)=1$, and let:
...
The Weil functional involves analytic function values at the locations of the zeros; without a Hardy, Sobolev, or strip holomorphic structure, these values cannot be controlled by the bare $L^2$ norm.
...
This round only proves the closure transfer:
...
This proposition is not a formal "smooth functions are usually dense"; the uniform norm acts on an unbounded closed strip, and requires holomorphy, vertical decay, vanishing at $s=0,1$, and unconditional zero evaluation to be simultaneously compatible.
...
3. Prove the density of the compactly supported Mellin image in the double-vanishing subspace, or find a smaller but still RH-equivalent complete core;
...
| ID | Status | This Round's Determination |
|---|---|---|
...
| `RH-W-02-TRANSFER-CORE` | `CLOSED_CONDITIONAL` | If dictionary positivity holds, it can be transferred to the complete core |
...
# 12. What Was Not Proven in This Round
...
What this round accomplished is:
...
It is recommended that the next round prioritize:
...
First, make the trace-negativity version of Bombieri/Clay and the covariance-positivity version of Lagarias into a term-by-term invertible conversion. Once completed, then process:
...
Otherwise, "in which completion space the core is dense" will still be distorted due to objects and notations not being fully aligned.
