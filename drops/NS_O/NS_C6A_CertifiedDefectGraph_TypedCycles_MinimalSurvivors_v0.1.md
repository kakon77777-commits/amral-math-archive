---
title: "Navier–Stokes C6-A：Certified Defect Graph、Typed Cycle Composition 與 Minimal Survivor Candidates"
...
# Navier–Stokes C6-A
# Certified Defect Graph、Typed Cycle Composition 與 Minimal Survivor Candidates

## 0. C6 正式啟動

C5-M 將 C5 的 residual state space壓成：
...
> 對 C5 finite defect graph做 SCC extraction。

但一開始就出現一個重要修正：

$$
\boxed{
\textbf{普通 label-level SCC 並不足以證明 PDE recurrent cycle。}
}
$$

原因：

- 一條 coarse edge可能只對 class內某 subtype成立；
- edge source / target有額外 metadata；
- 兩條 individually valid edges未必能首尾 compatibility；
- `not ruled out` self-loop不是 transition theorem；
- external regularity gates是 sink，不是 ordinary transitions；
- 很多 C5 routing本質上是 disjunctive hyperedge。
...
Navier–Stokes existence / smoothness仍是 Clay Millennium Prize open problem。

因此 C6 cycle analysis仍是研究程序中的 necessary-condition / defect-compatibility研究，

不是已知 global proof。

## 1.2 Grujić–Xu 2024

Grujić–Xu 的正式 2024 framework：
...
的主要 external kill gate。

## 1.3 Miller strain geometry

Miller middle-eigenvalue criterion限制 hypothetical finite-time blow-up的 strain geometry。

Miller strain-vorticity work再提供：
...
與 strain-vorticity operator regularity interfaces。
...
的重要 external/structural anchors。

## 1.4 Bradshaw–Tsai pressure provenance

whole-space local pressure expansion提供：

- local pressure；
- far pressure；
- harmonic far-field；

的 rigorous provenance。
...
不是 purely phenomenological graph node。
...
表示已進入 physical/legal regime後的五個 physical residual classes。

$A$ 暫不放入 physical SCC extraction，
...
可能表示 proof/theorem-entry failure，

不一定是 actual physical singularity mechanism。
...
arrows都投影成 ordinary directed edge，
...
於是 naive graph reading會說：

> physical survivor最後大概掉進 $\{G,P,H,F\}$ sink SCC。
...
Then coarse graph contains：

$$
X\leftrightarrow Y.
$$

However this does **not** imply existence of：
...
means implication is proved only under an extra gate：
...
means published regularity theorem closes the route when its antecedents hold。
...
For each class define compact/compactified metadata space：
...
is only an over-approximation to certified cycle structure。
...
endpoint/source fiber products nonempty。
...
A recurrent cycle can survive finite total budget if per-cycle toll shrinks fast enough。
...
For many states this becomes an AND–OR / hypergraph viability problem rather than ordinary SCC problem。
...
If regenerated derivative field is sparse，

external theorem gate closes。

## B6 — geometry/pressure exit

If forcing re-enters strain/pressure instead of H，

route exits the H/F cycle。
...
Naming may change depending on C6-B result。
...
cycle certification requires nonempty typed fiber product。

## G-REC

one compatible loop is weaker than recurrent invariant loop。
...
$A$ stays outside physical SCC until theorem-entry legality is proved。
...
$$
\boxed{
\textbf{C5-M 的 coarse SCC只能當 may-SCC，
不能直接當 PDE recurrent SCC。}
}
$$

如果把所有 C5 candidate arrows投影成普通 digraph，
...
只代表某 subtype / 某 condition下有 routing，

不代表所有 endpoint metadata都能接到下一條 edge。
...
以及 cycle compatibility fiber product：
...
且 recurrence / time / external-gate / debt條件全部合法，
...
都只是 candidate traps/cycles。

最重要的是：

$$
\boxed{
\textbf{目前尚沒有非平凡 recurrent PDE defect cycle被真正 certified。}
}
$$

這把 C6 的問題精確改寫成有限個 composition obligations。
...
$H\to F$ 已有 C5-J/L 的 turnover/roughness routing。
...
> high-order viscous / projected nonlinear forcing，
> 是否真的會重新生成一個 theorem-legal、
> 且整個 admissible window都 sign-thick 的下一代 defect？
...
C6 到這裡，已經從「找 cycle」變成：

$$
\boxed{
\textbf{證 cycle能不能真的首尾相接。}
}
$$
